...
- Prisma models and database tables for `users`, `roles`, `user_platform_role_assignments`, `tenant_memberships`, and `tenant_membership_role_assignments`
...
- Explicit role metadata that distinguishes role scope and effective auth behavior
...
- The shared `roles` table must remain the only role catalog used by both assignment paths.
