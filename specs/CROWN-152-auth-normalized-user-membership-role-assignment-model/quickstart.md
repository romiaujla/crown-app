...
- Use `PlatformUser` as the long-lived identity root when planning normalized Prisma changes.
- Use explicit platform-role assignments for `super_admin` handling.
- Use tenant memberships as the tenant anchor and tenant-role assignments as the actual tenant authorization grant.
