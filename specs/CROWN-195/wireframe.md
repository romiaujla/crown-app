...
- `TabsList` renders a single-row tab rail with a line-style treatment and active underline.
...
- Active: selected trigger is emphasized with stronger text and a clear bottom border.
...
- Desktop: `TabsList` sizes to content and keeps the trigger row compact along a bottom divider.
...
- Use semantic tokens only: `bg-card`, `text-foreground`, `text-muted-foreground`, `border-border`, `ring-ring`, and related opacity utilities.
- Keep labels in the approved `Inter` body family and reserve stronger emphasis for the active state only.
- Avoid raw color literals and keep the tab treatment aligned with the existing line-style navigation pattern used in shadcn-inspired surfaces.
