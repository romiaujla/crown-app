...
- For prompt-driven starts in the form `Start implementing <JIRA ISSUE>`, require Spec Kit artifacts before implementation unless the user explicitly adds `--skip-speckit`.
...
5. If the prompt does not include `--skip-speckit`, begin with `/specify`, then continue in this order only: `/plan`, `/tasks`, implementation, and pull request creation.
