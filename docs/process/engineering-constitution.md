...
- Release publication is manually triggered after eligible changes have landed on `main`; merges to `main` do not automatically publish a release.
...
- Each published release should aggregate the Jira-linked changes included since the previous release and surface those issue keys in release metadata or release notes.
