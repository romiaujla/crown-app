...
- Platform endpoints require `auth_class=super_admin` (resolved from the user's platform role assignment).
- Tenant admin endpoints require `auth_class=tenant_admin` and tenant context match.
- Tenant user endpoints require `auth_class=tenant_user` (or `tenant_admin`) and tenant context match.
