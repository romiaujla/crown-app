...
const main = async () => {
  ...
  const stats = filterStats(mergeStats(mergeStats(mergeStats(committed, workingTree), staged), untracked));
  ...
  const isSmallChange = stats.files.length <= SMALL_FILE_THRESHOLD && stats.total <= SMALL_LINE_THRESHOLD;
