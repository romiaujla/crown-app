...
describe("platform tenant slug availability contract", () => {
  it("returns 200 with an available result for super_admin", async () => {
    ...
    const app = buildApp({ platformTenantsRouter: createPlatformTenantsRouter({ getSlugAvailability }) });
  ...
  it("returns 401 for missing token", async () => {
    const app = buildApp({ platformTenantsRouter: createPlatformTenantsRouter({ getSlugAvailability: vi.fn() }) });

    const response = await request(app).post("/api/v1/platform/tenant/slug-availability").send({ slug: "acme-logistics" });
  ...
  it("returns 429 when rate limited", async () => {
    ...
    const app = buildApp({
      platformTenantsRouter: createPlatformTenantsRouter({ getSlugAvailability: vi.fn(), searchRateLimitMiddleware })
    });
