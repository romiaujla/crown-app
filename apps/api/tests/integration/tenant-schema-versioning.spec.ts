...
import {
  ...
  collectVersionsForTenant,
  type VersionRow,
} from '../helpers/tenant-provisioning-db.js';
...
describe('tenant schema versioning integration', () => {
  ...
  beforeEach(() => {
    ...
    findUnique.mockImplementation(
      async ({ where }: { where: { tenantId_version: { tenantId: string; version: string } } }) => {
  ...
  it('persists version rows for successful bootstrap', async () => {
    ...
    expect(collectVersionsForTenant(rows, 'tenant-1')).toEqual([
