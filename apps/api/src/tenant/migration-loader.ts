...
// Prisma-generated management-system filenames to keep version metadata human-readable.
const toDescription = (filename: string) =>
  filename
    .replace(/^\d+_/, '')
    .replace(/\.sql$/i, '')
    .replace(/_/g, ' ');

export const loadTenantMigrations = async (
  migrationsRoot = defaultMigrationsRoot,
): Promise<TenantMigrationDefinition[]> => {
