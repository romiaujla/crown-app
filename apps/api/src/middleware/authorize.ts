...
import { AuthErrorCode, type Role } from "../auth/claims.js";
...
export const authorize =
  ...
  (req: Request, res: Response, next: NextFunction) => {
    if (!req.auth) return sendAuthError(res, 401, AuthErrorCode.UNAUTHENTICATED, "Authentication required");

    const namespace = options.namespace ?? resolveNamespaceFromPath(req.path);
    if (!namespace) return sendAuthError(res, 403, AuthErrorCode.FORBIDDEN_ROLE, "No authorization policy matched route");

    if (options.allowedRoles && !options.allowedRoles.includes(req.auth.role)) {
      return sendAuthError(res, 403, AuthErrorCode.FORBIDDEN_ROLE, "Role is not permitted");
    }
    ...
    if (decision.reason === "forbidden_tenant") {
      return sendAuthError(res, 403, AuthErrorCode.FORBIDDEN_TENANT, "Tenant scope mismatch");
    }
    return sendAuthError(res, 403, AuthErrorCode.FORBIDDEN_ROLE, "Role is not permitted");
  };
