...
const resolveTargetTenant = (req: Request) =>
  req.header('x-tenant-id') ?? req.params.tenantId ?? null;

export const authorize =
  (options: AuthorizeOptions = {}) =>
  (req: Request, res: Response, next: NextFunction) => {
    if (!req.auth)
      return sendAuthError(res, 401, AuthErrorCodeEnum.UNAUTHENTICATED, 'Authentication required');
    ...
    if (!namespace)
      return sendAuthError(
        res,
        403,
        AuthErrorCodeEnum.FORBIDDEN_ROLE,
        'No authorization policy matched route',
      );
