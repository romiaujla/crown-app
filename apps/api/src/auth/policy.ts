...
const isTenantRole = (role: RoleEnum) =>
  role === RoleEnum.TENANT_ADMIN || role === RoleEnum.TENANT_USER;
...
export const evaluateAccess = (
  ...
  if (namespace === 'platform') {
    return claims.role === RoleEnum.SUPER_ADMIN
      ? { allow: true }
      : { allow: false, reason: 'forbidden_role' };
  }
