...
import {
  AuthResolutionFailureReasonEnum,
...
const toCurrentUserContext = async (
  ...
  return {
    ...
    tenant: tenant
      ...
      : null,
    targetApp:
      role === RoleEnum.SUPER_ADMIN ? AuthTargetAppEnum.PLATFORM : AuthTargetAppEnum.TENANT,
    routing: {
...
const mapLoginResolutionFailure = (reason: AuthResolutionFailureReasonEnum): LoginFailure => {
  switch (reason) {
...
export const prismaAuthService: AuthService = {
  async login(identifier, password) {
    ...
    const currentUser = await toCurrentUserContext(
      identity,
